...
MODEL_NAME = 'object_detection/Model_onlyCars_ssd'
...
PATH_TO_LABELS = os.path.join('object_detection', 'data', 'car_label_map.pbtxt')
...
def detect_objects(image_np, sess, detection_graph,image,depth,point_cloud,category_index):
    ...
    out = sess.run(
        ...
        feed_dict={image_tensor: image_np_expanded},options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),run_metadata=run_metadata)
...
if __name__ == '__main__':
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph, config=config) as sess:
            ...
            init.camera_resolution = sl.PyRESOLUTION.PyRESOLUTION_HD720    #'PyRESOLUTION_HD1080', 'PyRESOLUTION_HD2K', 'PyRESOLUTION_HD720', 'PyRESOLUTION_LAST', 'PyRESOLUTION_VGA'
            ...
            dump_steps=[22]
            with tf.contrib.tfprof.ProfileContext('/tmp/train_dir',trace_steps=range(10, 20), dump_steps=[22],enabled=True) as pctx:            
                pctx.add_auto_profiling('graph', opts, dump_steps)
                while True:
