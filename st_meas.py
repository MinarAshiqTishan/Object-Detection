...
def detect_objects(image_np, sess, detection_graph,image,depth,point_cloud,category_index):
    ...
    for i in range(num_detections):
         ...
         if score > 0.3:
             ...
             cv2.circle(image_np, ( int((x+right)/2), int((y + bottom)/2) ), 8, (125, 155, 21), thickness=2)
             ...
             if not np.isnan(distance) and not np.isinf(distance):
                 distance = str(float(distance))[0:4] #round() for int
                 ...
                 cv2.putText(image_np, "{0}:{1} m".format(display_str,distance),(int(centerx), int(centery)), cv2.FONT_HERSHEY_SIMPLEX, 1, (225, 255, 221),thickness=2,lineType=2)
        ...
        with tf.Session(graph=detection_graph, config=config) as sess:
            ...
            while True:
                ...
                if err == tp.PyERROR_CODE.PySUCCESS:
                    ...
                    #img = image.get_data()
                    image = cv2.resize(image,(1382,512))
                    frame_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    result_rgb = detect_objects(frame_rgb, sess, detection_graph,image,Z,point_cloud,category_index)
